...
<%
    ...
    Date validatedDate = GetterUtil.getDate(request.getAttribute("liferay-ui:app-view-entry:validatedDate"), DateFormatFactoryUtil.getDate(locale), null);
    ...
    <c:when test='<%= displayStyle.equals("icon") %>'>
        <div class="app-view-entry app-view-entry-taglib entry-display-style display-<%= displayStyle %> <%= showCheckbox ? "selectable" : StringPool.BLANK %> <%= cssClass %>" <%= AUIUtil.buildData(data) %> data-draggable="<%= showCheckbox ? Boolean.TRUE.toString() : Boolean.FALSE.toString() %>" data-title="<%= HtmlUtil.escapeAttribute(shortTitle) %>">
            ...
            <div class="entry-thumbnail" style="<%= thumbnailDivStyle %>">
                ...
                <c:if test="<%= ( (status != WorkflowConstants.STATUS_ANY) ) %>">
                    <%
                        request.setAttribute("aui:workflow-status:showIcon",false);
                        request.setAttribute("aui:workflow-status:showLabel",false);
                        request.setAttribute("aui:workflow-status:status",status);

                    %>
                    <jsp:include page="/html/publication/admin/taglib/workflow_status.jsp" />
                </c:if>
            </div>
            ...
            <div class="entry-metadata">
                ...
                <small>
                    ...
                    <dl>
                        ...
                        <c:if test="<%= ((status != WorkflowConstants.STATUS_ANY) && (status != 5 )) %>">
                            <dt>
                        ...
                        <c:if test="<%= validatedDate != null %>">
                            <dt>
                                <liferay-ui:message key="validated-date" />:
                            </dt>
                            <dd>

                                <%= HtmlUtil.escape(dateFormatDateTime.format(validatedDate)) %>
